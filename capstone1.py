...
from sqlalchemy import text # Required for Supabase connection
...
def create_usertable():
    with conn.session as s:
        s.execute(text('CREATE TABLE IF NOT EXISTS userstable (username TEXT PRIMARY KEY, password TEXT);'))
        s.commit()
...
def add_userdata(username, password):
    with conn.session as s:
        s.execute(
            text('INSERT INTO userstable (username, password) VALUES (:username, :password);'),
...
def login_user(username, password):
    df = conn.query('SELECT * FROM userstable WHERE username = :username AND password = :password;', 
            ...
            with st.form(key="register_form"):
                ...
                if register_button:
                    if new_password != confirm_password:
                        ...
                        df_check = conn.query('SELECT * FROM userstable WHERE username = :username;', 
